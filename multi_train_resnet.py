...
def load_backbone_ignore_fc(model: nn.Module, pretrain_pth: str):
    if not (pretrain_pth and os.path.isfile(pretrain_pth)):
        print(f"[Pretraining Loading] The path is invalid or does not exist:{pretrain_pth}")
        return
    ...
    print(f"[Pretraining Loading] Successfully loaded {len(new_sd)} weights; Skipped {len(skipped)};"
          f"Missing key {len(missing)} in model; unexpected key {len(unexpected)}")
...
def masked_partial_bce_with_logits(logits: torch.Tensor,
    ...
    loss_elem = F.binary_cross_entropy_with_logits(logits, targets, reduction='none')  # [B,C]
...
def main():
    train_csv = "/home/jliu3422/Dataset/label_train_multi.csv"
    train_dir = "/home/jliu3422/Dataset/train_multi"
    val_csv   = "/home/jliu3422/Dataset/label_val_cropped_select.csv"
    val_dir   = "/home/jliu3422/Dataset/val_cropped_select"
    csv_encoding = "gbk"
        ...
        for lr in lrs:
            for i, pth in enumerate(paths[args.arch]):
                for trick in tricks_com:
                    ...
                    fileHead = f"multi_train_{args.arch}_pth{i + 1}_{combined_trick}_{lr}"
                    ...
                    for epoch in range(epochs):
                        ...
                        for images, label, _names in tqdm(train_loader, desc=f"[{fileName}] Train {epoch}"):
                            ...
                            if not do_mix:
                                ...
                                gt_single = label.argmax(dim=1)         # [B]
                        ...
                        y_true = np.concatenate(all_true_multi, axis=0)   # [N,7] {0,1}
                    ...
                    results_csv = os.path.join(".", "new_results.csv")
